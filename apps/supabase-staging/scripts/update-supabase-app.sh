...
git-merge-subpath() {
    ...
    if (($? == 1)); then
        ...
        if git diff --cached --quiet; then
            ...
            git commit -m "enha(be): Merge $SOURCE_COMMIT:$SOURCE_PREFIX/ to $DEST_PREFIX/" \
...
# the latest changes
if ! git remote get-url supabase >/dev/null 2>&1; then
    git remote add -f -t master --no-tags supabase https://github.com/supabase/supabase.git
    echo -e "${COLOR_YELLOW}Added the supabase remote repository${COLOR_RESET}"
fi

echo -e "Fetching the latest changes from the supabase remote repository"
git fetch supabase

# transfer contents of the supabase/docker/ folder to local ./
echo -e "Running git-merge-subpath for source branch supabase/master (path: docker/) and target branch \
$(basename "$ROOT_DIR")/$(git symbolic-ref --short HEAD 2>/dev/null || true) \
(path: .)"

cd "$ROOT_DIR" && HUSKY=0 git-merge-subpath --squash supabase/master docker . && (cd - >/dev/null || true)
